...
function update()
    {
        echo "\n\e[91m\e[1m[+] Ashrarhack UPDATE UTILITY [+]\nUpdate in progress, please wait...\n\n$cln";
        system("git fetch origin && git reset --hard origin/master && git clean -f -d");
        echo $bold . $fgreen . "[i] Job finished successfully! Please Restart Ashrarhack \n" . $cln;
        exit;
    }
...
elseif ($ip == "fix")
  {
    echo "\n\e[91m\e[1m[+] Ashrarhack FiX MENU [+]\n\n$cln";
...
scanlist:
    ...
    system("clear");
    echo $bold . $blue . "
      |_ashrarhack___________________________________________________|
    ...
    if (!in_array($scan, array(
    ...
    else
      {
        ...
        elseif ($scan == "8")
          {
            ...
            if (count($array) > 0)
              {
                userinput("Do You Want Ashrarhack To Detect CMS Of The Sites? [Y/N]");
            ...
            if (strpos($srccd, "wp-content") !== false)
              {
                ...
                if ($wp_version != "")
                  ...
                  {
                    $reallink = $ipsl . $ip;
                    echo $red . "Failed \n\n[!] Ashrarhack could not determine the WordPress version of the target!";
                    echo "\n\n";
                    echo $bold . $yellow . "[*] Scanning Complete. Press Enter To Continue OR CTRL + C To Stop\n\n";
                    trim(fgets(STDIN, 1024));
                    goto scanlist;
        ...
        elseif ($scan == 'U' || $scan == 'u')
          {
            echo "\n\n" . $bold . $yellow . "-[ Ashrarhack Update Corner]-\n\n" . $cln;
            echo $bold . "[i] Fetching Stuffs .... \n" . $cln;
            $latestversion = readcontents("https://github.com/ashrarhacka409/WIA/version.txt");
            ...
            elseif ($rhversion == $latestversion)
              {
                echo $bold . $fgreen . "\n[i] You are already running the latest version of ashrarhack. \n\n" . $cln;
                goto scanlist;
              }
            else
              {
